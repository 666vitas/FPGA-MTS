...
本规范建立阶段不创建 releases 目录、不复制历史 bit，也不生成新 bit。
...
其中 bit 与 RELEASE_MANIFEST.md 为正式 release 的必需项；timing 和 test evidence 可保留原始报告或由 manifest 指向不可变证据。已有 release 不得原地替换 bit；任何重建都使用新的 release_name。
