...
class VivadoPlugin(FPGAVendorPlugin):
    ...
    def program_device(self, config: Dict[str, Any], flash_mode: bool = False) -> BuildResult:
        ...
        print(f"开始设备编程，模式: {'Flash' if flash_mode else 'JTAG'}")

        # 如果是Flash模式，先生成MCS文件
        if flash_mode:
            print("Flash模式：首先生成MCS文件...")
        ...
        try:
            generator = TCLScriptGenerator(config)
            tcl_script = generator.generate_program_device_script(flash_mode)
        except Exception as e:
            return BuildResult(
                success=False,
                artifacts={},
        ...
        # 执行编程
        script_name = "program_flash.tcl" if flash_mode else "program_jtag.tcl"
        result = self._run_vivado_tcl(tcl_script, script_name)

        # 添加编程特定的工件信息
        if result.success:
            if flash_mode:
                result.artifacts['flash_programmed'] = 'Flash烧录完成'
                result.artifacts['programming_mode'] = 'flash'
            else:
                result.artifacts['jtag_programmed'] = 'JTAG烧录完成'
                result.artifacts['programming_mode'] = 'jtag'

            # 添加编程配置信息
            programming_config = config.get('programming', {})
            if programming_config:
                result.artifacts.update({
                    'cable': programming_config.get('cable', 'xilinx_tcf'),
                    'target': programming_config.get('target', 'localhost:3121'),
                    'bitfile': programming_config.get('bitfile', 'auto_detected')
                })
