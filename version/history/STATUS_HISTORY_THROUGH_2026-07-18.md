# FPGA-MTS STATUS History

> **HISTORICAL / NOT ACTIVE**：本文件保存从 `version/STATUS.md` 移出的历史状态。它不能决定当前 Stage、Gate、参数、接线或下一步；当前状态只读 `version/STATUS.md`。
...
- Current Blocker：真实 loaded PZT 节点的 SCAN 电压、频率、无异常跳变/saturation 和对应光谱/MTS error 响应尚未验证。
- 该阻塞属于 loaded PZT 接线与实验证据，不是当前已知 Python、RTL、寄存器或 bitstream 缺陷。

### HV-1B 最新硬件证据

- [USER HARDWARE VERIFIED] 用户明确确认：在 PZT 断开、使用当前软件预补偿后，上位机设定的 OUT2 电压与板上真实输出一致。HV-1B 不再要求重复空载 SCAN 复测。
- 用户未在本轮提供独立的 center/Vpp/frequency、scope load、coupling、probe ratio 数值；记录为用户硬件通过声明，不虚构缺失测量数据。
- [AUTOMATED VERIFIED] 当前 host 仍使用统一 OUT2 absolute/amplitude 校准路径。本轮修改后 `tabnanny` 和 `py_compile` 通过；聚焦测试 `8 passed, 87 deselected`；`137 tests collected`；完整 software tests `137 passed`。

### Gate 边界

- [NOT VERIFIED] loaded PZT 节点真实电压、光谱扫描响应、真实 Windows GUI、人工锁点、Kp=0 无扰切换和 P-only。
- HV-2 只允许 loaded PZT 的低速 SCAN-only 观测。本 Gate 禁止 `HOLD SELECTED COUNT`、`LOCK HERE`、`APPLY P`、polarity 变更或非零 Kp/Ki。
- [AUTOMATED VERIFIED] 现有 host 已提供本 Gate 所需的 identity、SAFE、SCAN safe-range、MODE/ENABLE、OUT2 readback、saturation 和 capture 观测。本轮唯一独立工程复核发现 1 个 High：从默认 50 Hz 改为 2 Hz 时 Lock View capture window 未自动同步，可能只捕获约 4% 周期。现已让 scan frequency/capture length 变化自动重算 decimation；`2 Hz / 2048 points -> 30518`，window 约 `0.5 s`。复核无其他 Blocker/High。

### 下一步唯一动作

按 `software/redpitaya_lock_host/docs/LOADED_PZT_SCAN_SOP.md` 只执行一次 loaded PZT 低速 SCAN：在 SAFE 下将 `OUT2 -> 激光器专用 PZT/Scan 输入 + 示波器 1 MΩ/Hi-Z`，确认该 PZT 输入允许 `0.600~0.900 V`，使用 `center=0.770 V / amplitude=0.080 V / 2 Hz / Kp=0 / Ki=0`，只记录 loaded-node 电压、频率、CH1/CH3/CH4、MODE/ENABLE/readback 和 saturation，然后立即 SAFE。
