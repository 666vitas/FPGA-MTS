...
module tb_deterministic_lock_acquisition;
    ...
    initial begin
        ...
        bus.wen = 1'b1;
        #1;
        check("ABORT suppresses an otherwise-valid same-cycle trigger", !acq_trigger && acq_abort);
        @(posedge clk);
        #1;
        check("same-cycle ABORT drives exact SAFE output", selected_out2 == 14'sd0);
        @(negedge clk);
        bus.wen = 1'b0;
        bus.addr = 32'd0;
        bus.wdata = 32'd0;
        bus_read(REG_EVENT_INFO, read_data);
        check("same-cycle ABORT records ABORTED rather than TRIGGERED", read_data[3:1] == 3'd3);
