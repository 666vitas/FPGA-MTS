...
set build_dir [file join $evidence build_final]
set synth_name l1_candidate_synth_final_20260911
set impl_name l1_candidate_impl_final_20260911
...
set identity [open [file join $evidence final_build_identity.txt] w]
...
puts $identity "ENABLE_DAISY=0; DNA clock=adc_clk/16; PHYS_OPT=disabled after crash; ROUTE=Explore"
close $identity
...
puts "FINAL_STATUS=[get_property STATUS [get_runs $impl_name]]"
if {![string match "*Complete*" [get_property STATUS [get_runs $impl_name]]]} { error "implementation failed" }
open_run $impl_name
report_timing_summary -delay_type min_max -report_unconstrained -check_timing_verbose \
  -file [file join $evidence final_timing_summary.rpt]
check_timing -verbose -file [file join $evidence final_check_timing.rpt]
report_route_status -file [file join $evidence final_route_status.rpt]
report_drc -file [file join $evidence final_drc.rpt]
report_cdc -file [file join $evidence final_cdc.rpt]
report_methodology -file [file join $evidence final_methodology.rpt]
report_utilization -hierarchical -file [file join $evidence final_utilization.rpt]
write_checkpoint -force [file join $evidence final_routed.dcp]
close_project
exit
